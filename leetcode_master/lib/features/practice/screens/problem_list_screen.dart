...
class ProblemListScreen extends ConsumerWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          questionSetsAsync.when(
            data: (sets) => _buildFilterButton(context, ref, sets, selectedSetId),
  ...
  Widget _buildFilterButton(
    ...
    final isFiltered = selectedSetId != null;
    ...
    return PopupMenuButton<String?>(
      ...
      onSelected: (value) {
        ref.read(selectedQuestionSetProvider.notifier).state = value;
      },
      itemBuilder: (context) => [
        PopupMenuItem<String?>(
          ...
          child: Row(
            children: [
              Icon(
                selectedSetId == null ? Icons.check : Icons.list,
                size: 18,
                color: selectedSetId == null ? LCMColors.primary : LCMColors.textSecondary,
              ),
              const SizedBox(width: 12),
              Text(
                'All Problems',
                style: TextStyle(
                  color: selectedSetId == null ? LCMColors.primary : LCMColors.textPrimary,
                  fontWeight: selectedSetId == null ? FontWeight.w600 : FontWeight.normal,
                ),
              ),
            ],
          ),
        ...
        ...sets.map((set) => PopupMenuItem<String?>(
              value: set.id,
              child: Row(
                children: [
                  Icon(
                    selectedSetId == set.id ? Icons.check : Icons.bookmark_outline,
                    size: 18,
                    color: selectedSetId == set.id ? LCMColors.primary : LCMColors.textSecondary,
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      set.name,
                      style: TextStyle(
                        color: selectedSetId == set.id ? LCMColors.primary : LCMColors.textPrimary,
                        fontWeight: selectedSetId == set.id ? FontWeight.w600 : FontWeight.normal,
                      ),
                    ),
                  ),
                ],
  ...
  Widget _buildProblemList(BuildContext context, List<Problem> problems, String? selectedSetId) {
    // Apply filter if a set is selected
    final filteredProblems = selectedSetId == null
        ? problems
        : problems.where((p) => p.questionSetIds.contains(selectedSetId)).toList();
    ...
    final easyProblems =
        filteredProblems.where((p) => p.difficulty == Difficulty.easy).toList();
    final mediumProblems =
        filteredProblems.where((p) => p.difficulty == Difficulty.medium).toList();
    final hardProblems =
        filteredProblems.where((p) => p.difficulty == Difficulty.hard).toList();
